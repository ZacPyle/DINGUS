...
def plot_mesh(myMesh: 'mesh_class.Mesh'):
    '''
    ...
    if myMesh.dim in PLOTTING_DISPATCH:
        return PLOTTING_DISPATCH[myMesh.dim](myMesh)
    else:
...
def plot_mesh_1D(myMesh: 'mesh_class.Mesh'):
    '''
...
def plot_mesh_2D(myMesh: 'mesh_class.Mesh'):
    '''
    ...
    # Extract top level mesh structure information
    num_elements = myMesh.num_elements
    num_nodes    = myMesh.num_nodes
    dim          = myMesh.dim

    # Loop through elements and plot element boundaries and quadrature nodes
    

def plot_mesh_3D(myMesh: 'mesh_class.Mesh'):
    '''
