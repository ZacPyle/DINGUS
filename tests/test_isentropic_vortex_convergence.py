...
# The test-required run length, owned here so control.yaml is free for interactive use. Kept short: the
# exact solution is the vortex translated by (U_INF, V_INF) * final_time, so any value is valid, and a
# small one keeps each degree in the sweep cheap.
...
def _plot_convergence(degrees: list, errors: list, slope: float) -> Path:
    ...
    ax.legend()
    fig.tight_layout()
    out_path = FIG_DIR / "isentropic_vortex_convergence.png"
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
    return out_path


def _run_sweep(degrees: list):
