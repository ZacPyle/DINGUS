# tests/test_isentropic_vortex_roe_llf.py
'''
Riemann-solver comparison on the isentropic vortex: LLF (Rusanov) vs Roe.
...
IMPORTANT nuance (see test_isentropic_vortex_convergenceFAST.py's docstring): on this [0,10]^2 domain the
...
Run `pytest -s tests/test_isentropic_vortex_roe_llf.py` to see the comparison table; a two-curve semilog
convergence plot is saved under tests/test_case_isentropic_vortex/outputs/figures/.
'''
...
CASE_DIR  = Path(__file__).resolve().parent / "test_case_isentropic_vortex"
CTRL_FILE = CASE_DIR / "control.yaml"
FIG_DIR   = CASE_DIR / "outputs" / "figures"

# The test-required run length, owned here so control.yaml is free for interactive use (see the
# convergence test for the same pattern). Short: the exact solution is the translated vortex.
...
def _l2_error(poly_deg: int, riemann_solver: str) -> float:
    ...
    cfg.time_stepping.final_time = VORTEX_FINAL_TIME # test owns its timing; control.yaml is free
...
def _print_table(degrees, llf, roe) -> None:
    ...
    table = Table(title="Isentropic vortex: LLF vs Roe Riemann solver")
...
def _plot_comparison(degrees, llf, roe) -> Path:
    ...
    ax.set_title("Isentropic vortex: LLF vs Roe interface dissipation")
    ax.set_xticks(degrees)
    ax.grid(True, which="both", ls=":", alpha=0.6)
    ax.legend()
    fig.tight_layout()
    out_path = FIG_DIR / "isentropic_vortex_roe_vs_llf.png"
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
    return out_path


@pytest.mark.numerics          # a property of the SCHEME (dissipation of two Riemann solvers), not of the physics
@pytest.mark.slow              # ...but it convects the vortex, so it is minutes, not seconds
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_isentropic_vortex_roe_vs_llf():
    ...
    #    the thresholds in test_isentropic_vortex_convergenceFAST.py, applied to BOTH solvers.
