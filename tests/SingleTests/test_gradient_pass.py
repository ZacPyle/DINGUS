...
'''
Unit test for the BR1 gradient pass (residual.compute_gradient), which computes g = grad(q) and
...
# (P+1, P+1, 1, 2)). compute_gradient is physics-agnostic, so the scalar case is the simplest driver.
CASE_DIR  = Path(__file__).resolve().parent.parent / "test_case_advection_scalar"
...
def test_gradient_constant_field_is_zero():
    ...
    residual.compute_gradient(mesh, cfg)
